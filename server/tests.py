...
class ServerTests(TestCase):
    ...
    def test_multiple_games(self):
        user1idcat = self.creatUser("user1", "user1")
        user2idmouse = self.creatUser("user2", "user2")
        user3idcat = self.creatUser("user3", "user3")
        user4idmouse = self.creatUser("user4", "user4")
        responsecat = self.clientCat.get(reverse('clean_orphan_games'))
        #login
        response = self.login("user1", "user1",self.cat1)
        response = self.login("user2", "user2",self.mouse1)
        response = self.login("user3", "user3",self.cat2)
        response = self.login("user4", "user4",self.mouse2)
        #create game
        response = self.cat1.get(reverse('create_game'))#follow redirection
        self.assertEqual(response.status_code, 200)
        #join game
        response = self.mouse1.get(reverse('join_game'))
         #create game
        response = self.cat2.get(reverse('create_game'))#follow redirection
        self.assertEqual(response.status_code, 200)
        #join game
        response = self.mouse2.get(reverse('join_game'))

        #move 2 ->11, movimiento valido para el juego de cat1 y mouse 1
        moveDict={}
        moveDict['origin']=2
        moveDict['target']=11
        response = self.cat1.post(reverse('move'), moveDict)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'catUser = %s (%d)'%("user1", user1idcat),response.content)
        self.assertIn(b'mouseUser = %s (%d)'%("user2", user2idmouse),response.content)

        #move 2 ->11, mismo movimiento que antes pero en otro juego distinto
        moveDict={}
        moveDict['origin']=2
        moveDict['target']=11
        response = self.cat2.post(reverse('move'), moveDict)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'catUser = %s (%d)'%("user3", user3idcat),response.content)
        self.assertIn(b'mouseUser = %s (%d)'%("user4", user4idmouse),response.content)
        
        #mouse moves en el juego 1
        moveDict['origin']=59#not needed
        moveDict['target']=52
        response = self.mouse1.post(reverse('move'), moveDict)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'mouseUser = %s (%d)'%("user2", user2idmouse),response.content)
        ...
        #Realizo un movimiento que ahora seria valido en el primer juego, pero no en el segundo
        moveDict['origin']=0#not needed
        moveDict['target']=45
        response = self.mouse2.post(reverse('move'), moveDict)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Cannot create a move',response.content)

        #mouse moves en el juego 2 con un movimiento valido
        moveDict['origin']=59#not needed
        moveDict['target']=52
        response = self.mouse2.post(reverse('move'), moveDict)
        self.assertEqual(response.status_code, 200)
