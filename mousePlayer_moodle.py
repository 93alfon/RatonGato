...
class MousePlayer():
    ...
    def wait_loop(self, seconds):
        ...
        while True:
            ...
            response = self.clientMouse.post('/server/status_board/', {'value': '1',}, HTTP_X_REQUESTED_WITH='XMLHttpRequest')
    ...
    def parse_cats(self):
        """http://pythex.org/"""
        response = self.clientMouse.post('/server/status_board/', {'value': '1',}, HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        searchString='<td id=id_(\d+) onclick="getID\(this\);" >\s+<img class="token" src="/static/images/cat.png" alt="cat" style="opacity: 0.7"/>'
...
#join game
response = mousePlayer.clientMouse.get(reverse('wait_player'))
moveDict={}
while True:
